...
function loadDisplayName() {
  ...
  // First, check if there's a saved display name in localStorage
  const savedDisplayName = localStorage.getItem("userDisplayName");

  // If no saved display name, try to get the username from sessionStorage
  if (!savedDisplayName) {
    // Fetch user details from backend or sessionStorage
    const storedUserId = sessionStorage.getItem("user_id");

    if (storedUserId) {
      // TODO: Implement backend call to fetch username
      // For now, we'll use a placeholder approach
      fetch(
        `http://localhost/WebDev/devhivespace/api/user/get_username.php?user_id=${storedUserId}`
      )
        ...
        .then((userData) => {
          if (userData.username) {
            ...
            updateDisplayNameInProfile(userData.username);
          }
        })
        .catch((error) => {
    ...
    // If saved display name exists, use that
...
function updateDisplayNameInProfile(displayName) {
  ...
  const profileNameElements = [document.querySelector(".profile-details h2")];
  ...
  input.onchange = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const userProfileAvatar = document.querySelector(
          "#user-profile-avatar"
        );
...
function initializeAccountSettings() {
  ...
  const twoFactorToggle = document.querySelector(
    ".setting-item:nth-child(4) .toggle-switch"
  );
  twoFactorToggle.addEventListener("click", () =>
    toggleSwitch(twoFactorToggle)
  );
}
...
function initializeNotificationSettings() {
  const notificationToggles = document.querySelectorAll(
    ".settings-section:nth-child(2) .toggle-switch"
  );
...
function initializePrivacySettings() {
  const profileVisibilityDropdown = document.querySelector(
    ".settings-section:nth-child(3) .dropdown-btn"
  );
  const dataSharingToggle = document.querySelector(
    ".settings-section:nth-child(3) .toggle-switch"
  );
  const managePermissionsBtn = document.querySelector(
    ".settings-section:nth-child(3) .action-btn"
  );

  profileVisibilityDropdown.addEventListener(
    "click",
    handleProfileVisibilityDropdown
  );
  dataSharingToggle.addEventListener("click", () =>
    toggleSwitch(dataSharingToggle)
  );
...
function initializeAdvancedSettings() {
  const timeZoneDropdown = document.querySelector(
    ".settings-section:nth-child(4) .dropdown-btn:nth-child(1)"
  );
  const languageDropdown = document.querySelector(
    ".settings-section:nth-child(4) .dropdown-btn:nth-child(2)"
  );
...
function handleAccountDeletion() {
  if (
    ...
    if (
      prompt('Please type "DELETE" to confirm account deletion:') === "DELETE"
    ) {
...
function saveAllSettings() {
  const settings = {
    ...
    email: document.querySelector('input[placeholder="user.example@gmail.com"]')
      .value,
    twoFactorEnabled: document
      .querySelector(".setting-item:nth-child(4) .toggle-switch")
      .classList.contains("active"),
    notifications: {
      email: document
        .querySelector(
          ".settings-section:nth-child(2) .toggle-switch:nth-child(1)"
        )
        .classList.contains("active"),
      push: document
        .querySelector(
          ".settings-section:nth-child(2) .toggle-switch:nth-child(2)"
        )
        .classList.contains("active"),
      performance: document
        .querySelector(
          ".settings-section:nth-child(2) .toggle-switch:nth-child(3)"
        )
        .classList.contains("active"),
      engagement: document
        .querySelector(
          ".settings-section:nth-child(2) .toggle-switch:nth-child(4)"
        )
        .classList.contains("active"),
      scheduled: document
        .querySelector(
          ".settings-section:nth-child(2) .toggle-switch:nth-child(5)"
        )
        .classList.contains("active"),
    },
    privacy: {
      profileVisibility: document
        .querySelector(".settings-section:nth-child(3) .dropdown-btn")
        .textContent.trim(),
      dataSharing: document
        .querySelector(".settings-section:nth-child(3) .toggle-switch")
        .classList.contains("active"),
    },
    advanced: {
      timeZone: document
...
function loadSavedProfileAvatar() {
  ...
  const existingProfileImage = profileAvatarContainer.querySelector("img");

  if (savedAvatar) {
    if (existingProfileImage) {
      // If image already exists, update its source
      existingProfileImage.src = savedAvatar;
    } else {
      // If no image exists, create a new one
      const newAvatarImg = document.createElement("img");
      newAvatarImg.src = savedAvatar;
      newAvatarImg.id = "profile-image";
      newAvatarImg.alt = "Profile Avatar";
      profileAvatarContainer.appendChild(newAvatarImg);
    }
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener("click", function (e) {
      ...
      // Build logout URL with session ID in query string
      const logoutUrl = `/api/auth/logout.php?PHPSESSID=${sessionId}`;
      ...
      fetch(logoutUrl, {
        ...
        .catch(() => {
          // Always remove session ID and redirect, even on error
          localStorage.removeItem('PHPSESSID');
          window.location.href = "/login/index.html";
        });
    });
