...
async function loadPosts(offset = 0, filter = 'all post') {
    ...
    try {
        ...
        const url = new URL('/copy/pages/api/posts/get-post.php', window.location.href);
...
function openShareModal(post) {
    ...
    modal.querySelector('.share-original-content').innerHTML =
        `<span style="color:#555;">Originally posted by <b>${escapeHTML(post.author || 'Anonymous')}</b></span><br>
         <span>${escapeHTML(post.content)}</span>`;
    modal.querySelector('.share-link-input').value = generatePostLink(post);
    // ...add event listeners as before...
}

// Example function to share a post via API
async function sharePostToBackend(postId, userId, platform = 'devhive') {
    const response = await fetch('/api/posts/share.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ post_id: postId, user_id: userId, platform })
    });
    const result = await response.json();
    if (result.status === 'success') {
        showNotification('Post shared successfully!');
    } else {
        showNotification('Failed to share post: ' + result.message);
    }
}
