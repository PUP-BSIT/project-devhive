...
if (isset($_POST['allow'])) {
    if (!isset($_SESSION['user_id'])) {
        ...
        // Get provider name from database (assuming you have it in oauth_clients)
        $stmt = $conn->prepare("SELECT provider_name FROM oauth_clients WHERE client_id = ?");
        $stmt->bind_param("s", $client_id);
        $stmt->execute();
        ...
        $redir = $redirect_uri;
        $redir .= (strpos($redir, '?') === false ? '?' : '&') . "provider=" . urlencode($provider_name) . "&token=$token";
