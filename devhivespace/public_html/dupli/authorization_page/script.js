document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const res = await fetch(`/api/oauth/oauth_authorize.php?client_id=${encodeURIComponent(client_id)}&redirect_uri=${encodeURIComponent(redirect_uri)}&provider=${encodeURIComponent(provider)}`, { credentials: 'include' });
    ...
    form.addEventListener('submit', async function(e) {
      ...
      try {
        const resp = await fetch('/api/oauth/oauth_authorize.php', {
          method: 'POST',
          credentials: 'include',
