...
try {
    ...
    $query = "
        SELECT 
            ...
            NULL as target_type,
            u.profile_image_id,
            mf.filename as profile_image_filename
        FROM post p
        LEFT JOIN user u ON p.user_id = u.user_id
        LEFT JOIN media_files mf ON u.profile_image_id = mf.id
        ...
        SELECT 
            ...
            s.target_type,
            su.profile_image_id,
            mf2.filename as profile_image_filename
        FROM share s
        JOIN post p ON s.post_id = p.post_id
        LEFT JOIN user u ON p.user_id = u.user_id
        LEFT JOIN user su ON s.user_id = su.user_id
        LEFT JOIN media_files mf2 ON su.profile_image_id = mf2.id
        LEFT JOIN post_image pi ON p.post_id = pi.post_id
        LEFT JOIN post_video pv ON p.post_id = pv.post_id
        GROUP BY s.share_id
    ...
    // Add profile_image_url to each post
    foreach ($posts as &$row) {
        // For original posts, use the author's profile image
        if (empty($row['share_id'])) {
            if (!empty($row['profile_image_filename'])) {
                $row['author_profile_image_url'] = '/uploads/avatars/' . $row['profile_image_filename'];
            } else {
                $row['author_profile_image_url'] = '/assets/human.png';
            }
            $row['profile_image_url'] = $row['author_profile_image_url'];
        } else {
            // For shares, use the sharer's profile image
            if (!empty($row['profile_image_filename'])) {
                $row['shared_by_profile_image_url'] = '/uploads/avatars/' . $row['profile_image_filename'];
            } else {
                $row['shared_by_profile_image_url'] = '/assets/human.png';
            }
            $row['profile_image_url'] = $row['shared_by_profile_image_url'];
        }
    }
